    ...
    <div class="flex-col w-full md:flex md:flex-row md:min-h-screen">
        <div @click.away="open = false" class="flex flex-col flex-shrink-0 w-full text-gray-700 bg-white md:w-64 dark-mode:text-gray-200 dark-mode:bg-gray-800" x-data="{ open: false }">
            ...
            <nav :class="{'block': open, 'hidden': !open}" class="flex-grow px-4 pb-4 md:block md:pb-0 md:overflow-y-auto">
                <x-jet-nav-link href="{{ route('admin.books.index') }}" :active="request()->routeIs('admin.books.index')">
                    {{ __('Books') }}
                </x-jet-nav-link>
                <x-jet-nav-link href="{{ route('admin.authors.index') }}" :active="request()->routeIs('admin.authors.index')">
                    {{ __('Authors') }}
                </x-jet-nav-link>
                <x-jet-nav-link href="{{ route('admin.genres.index') }}" :active="request()->routeIs('admin.genres.index')">
                    {{ __('Genres') }}
                </x-jet-nav-link>
